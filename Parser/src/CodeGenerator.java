...
//        try {
            if (!returnSeen) {
                ...
                System.out.println(Color.ANSI_RED + "SEMANTIC WARNING in Line " + Scanner.line + " :\n" + Color.ANSI_BLUE + "Unreachable statement" + Color.ANSI_RESET);
                SS.pop();
            }
//        } catch (Exception e) {
//            System.out.println("Panic mode failed, genarated code till now:");
...
//        try {
            if (tokens[0].name.matches("\\d+\\.\\d+")) {
                System.out.println(Color.ANSI_RED + "SEMANTIC ERROR in Line " + Scanner.line + " Character " + (Scanner.pointer + 1) +
                        " :\n" + Color.ANSI_BLUE + "Type mismatched. float number cast to int" + Color.ANSI_RESET);
